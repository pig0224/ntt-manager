...
#	Description: NTT Manager CLI
#	Version: 1.0.0
#	Author: NTT Manager Team
#=================================================

cli_ver="1.0.0"
github="raw.githubusercontent.com/pig0224/ntt-manager/main"
...
Install_Manager(){
	echo "===================================================="  
	echo "========= Start installing the NTT Manager ========="
	echo "===================================================="

	export PATH=${NODE_DIR}/bin:$PATH

	if command -v ntt &>/dev/null; then
		echo "Complated, ntt Manager is installed."
		read -n 1 -p "Press any key back to menu..."
		start_menu
		return 0
	fi
	...
	npm install -g ntt-manager
	if [ $? -eq 0 ]; then
		ln -sf /opt/node-v20.11.1-linux-x64/bin/ntt /usr/local/bin/
		echo "NTT Manager Installed successfully"
	else
		echo "NTT Manager Installation failed, please check the error message above."
		read -n 1 -p "Press any key back to menu..."
		start_menu
		return 1
	fi
	...
	if command -v ntt &>/dev/null; then
		local status=$(ntt status 2>/dev/null)
...
Uninstall_Manager(){
	echo "=================================================="
	echo "========= Start uninstalling NTT Manager ========="
	echo "=================================================="
	...
	if [ ! -d "${NODE_DIR}" ] && ! command -v node &>/dev/null; then
		echo "Complated, NTT Manager is not installed."
		read -n 1 -p "Press any key back to menu..."
		start_menu
		return 0
	fi

	npm uninstall -g ntt-manager >/dev/null 2>&1

	rm -f /usr/local/bin/ntt
	rm -f /usr/local/bin/node
	rm -f /usr/local/bin/npm
	rm -rf "${NODE_DIR}"

	echo "Complated, NTT Manager has been uninstalled."
	read -n 1 -p "Press any key back to menu..."
	start_menu
}
...
Start_Manager(){
	echo "====================================="
	echo "========= Start NTT Manager ========="
	echo "====================================="

	export PATH=${NODE_DIR}/bin:$PATH

	if ! command -v ntt &>/dev/null; then
		echo "NTT Manager has not been installed. Please Install Manager."
		read -n 1 -p "Press any key back to menu..."
		start_menu
		return 1
	fi
	...
	if [[ ! "${status}" =~ "online" ]]; then
		...
		echo -e "${Tip} ntt Manager is already running"
	fi
...
Stop_Manager(){
	...
	if [[ "${status}" =~ "online" ]]; then
		...
		echo -e "${Tip} NTT Manager is not running"
	fi
...
start_menu(){
clear
echo && echo -e "NTT Manager CLI ${Red_font_prefix}[v${cli_ver}]${Font_color_suffix}
...
————————————————————————————————" && echo
	...
	# 	echo -e " Status: ${Green_font_prefix}Not installed${Font_color_suffix} NTT Manager ${Red_font_prefix} Please install. ${Font_color_suffix}"
	# else
